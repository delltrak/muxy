...
final class GitWorktreeService: GitWorktreeListing, Sendable {
    ...
    func isGitRepository(_ path: String) async -> Bool {
        guard let result = try? await runGit(repoPath: path, arguments: ["rev-parse", "--is-inside-work-tree"]) else {
            return false
        }
    ...
    func listWorktrees(repoPath: String) async throws -> [GitWorktreeRecord] {
        let result = try await runGit(repoPath: repoPath, arguments: ["worktree", "list", "--porcelain"])
    ...
    func addWorktree(repoPath: String, path: String, branch: String, createBranch: Bool) async throws {
        ...
        let result = try await runGit(repoPath: repoPath, arguments: args)
        guard result.status == 0 else {
            throw GitWorktreeError.commandFailed(
    ...
    private func runGit(repoPath: String, arguments: [String]) async throws -> GitRunResult {
        let result = try await GitProcessRunner.runGit(repoPath: repoPath, arguments: arguments)
        return GitRunResult(status: result.status, stdout: result.stdout, stderr: result.stderr)
    }
}
